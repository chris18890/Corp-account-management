...
function Create-Mailbox-OnPrem {
    param(
    [string]$UserName
    ,[string]$realname,[string]$SharedEquipmentRoom,[string]$Capacity
    )
    #================================================================
        ...
        try {
            ...
            switch ($SharedEquipmentRoom) {
                "S" {
                    ...
                    Set-ADObject -Identity "CN=$GroupName,$SharedGroupsOU,$EndPath" -protectedFromAccidentalDeletion $True -Server $DCHostName
                ...
                "E" {
                    ...
                    Set-ADObject -Identity "CN=$GroupName,$EquipmentGroupsOU,$EndPath" -protectedFromAccidentalDeletion $True -Server $DCHostName
                    Set-ADObject -Identity "CN=$UserName,$OUPath" -protectedFromAccidentalDeletion $True -Server $DCHostName
                    if ($AdminID) {
                        if ($Managed -eq "M") {
                            $Assistant = $AdminID + " (M)"
                ...
                "R" {
                    ...
                    Set-ADObject -Identity "CN=$GroupName,$RoomGroupsOU,$EndPath" -protectedFromAccidentalDeletion $True -Server $DCHostName
                    Set-ADObject -Identity "CN=$UserName,$OUPath" -protectedFromAccidentalDeletion $True -Server $DCHostName
                    if ($Capacity) {
                ...
                default {
                    ...
                    if ($Dept -eq "IT" -and $HiPriv -eq "Y") {
                        ...
                        Write-Log ""
                        .\CreateHiPrivITUser.ps1 -FirstName $FirstName -LastName $LastName -UserName $UserName -UserPassword $UserPassword -EmailSuffix $EmailSuffix -PrivLevel $PrivLevel -Description $Description -Dept $Dept -Company $Company -LogFile $LogFile -O365 $O365 -DCHostName $DCHostName -Manager $UserName -Requester $Requester -SMTPServer $SMTPServer -EmailFrom $EmailFrom -PasswordLength $PasswordLength
                    }
                }
            }
            if ($O365 -eq "E") {
            ...
            if ($O365 -eq "H") {
                ...
                Write-Log "Calling Create-Mailbox function with the following parameters:"
                Write-Log "UserName: $UserName, realname: $RealName, SharedEquipmentRoom: $SharedEquipmentRoom, Capacity: $Capacity"
                $enabledMailboxes += Create-Mailbox -UserName $UserName -realname $RealName -SharedEquipmentRoom $SharedEquipmentRoom -Capacity $Capacity
                # Send email to requester and manager with new user's name & password
                if ($SharedEquipmentRoom -eq "N") {
                    Send-UserEmail -UserName $UserName -password $UserPassword -requester $Requester -manager $Manager
                }
        ...
        Foreach ($mailbox in $EnabledMailboxes) {
            ...
            if ($MBX) {
                if ($Mailbox.SharedEquipmentRoom) {
                    ...
                    Write-Log $logMsg
                    Update-Mailbox $mailbox.Alias $Mailbox.SharedEquipmentRoom $Mailbox.Capacity
                } elseif (!$Mailbox.SharedEquipmentRoom -and !$Mailbox.Capacity) {
                    $logmsg = "Updating Mailbox: " + $Mailbox.Alias
                    Write-Log $logMsg
                    Update-Mailbox $mailbox.Alias $Mailbox.SharedEquipmentRoom $Mailbox.Capacity
