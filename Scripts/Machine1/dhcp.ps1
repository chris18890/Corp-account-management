...
Write-Log -LogFile $LogFile -LogString ("=" * 80)
Write-Log -LogFile $LogFile -LogString "Processing commenced, running as user '$env:USERNAME'"
...
Write-Log -LogFile $LogFile -LogString ("=" * 80)
Write-Log -LogFile $LogFile -LogString "Note - Script must be run TWICE on this server - first to do the domain join then again to set up DHCP"
Write-Log -LogFile $LogFile -LogString ("=" * 80)
        ...
        try {
            if ($Platform -eq "Local") {
                ...
                try {
                    ...
                    Write-Log -LogFile $LogFile -LogString "$Domain\ADM_Task_DHCP_Admins is already a member of DHCP Administrators" -ForegroundColor Green
                }
                try {
                    Add-LocalGroupMember -Group "DHCP Users" -Member "$Domain\ADM_Task_DHCP_Users" -ErrorAction Stop
                } catch [Microsoft.PowerShell.Commands.MemberExistsException] {
                    Write-Log -LogFile $LogFile -LogString "$Domain\ADM_Task_DHCP_Users is already a member of DHCP Users" -ForegroundColor Green
                }
            ...
            For ($i=0; $i -le $IPNet.Length -1; $i++){
                    ...
                    try {
                        ...
                        switch ($_.Exception.ErrorCode) {
                            20052 { # 'The specified object already exists'
                                Write-Log -LogFile $LogFile -LogString "Scope $Site[$i] already exists" -ForegroundColor Green
                            }
                            default {
                ...
                try {
                    ...
                    switch ($_.Exception.ErrorCode) {
                        8305 { # 'The specified object already exists'
                            Write-Log -LogFile $LogFile -LogString "Site $Site[$i] already exists" -ForegroundColor Green
                        }
                        default {
                ...
                try {
                    ...
                    switch ($_.Exception.ErrorCode) {
                        8305 { # 'The specified object already exists'
                            Write-Log -LogFile $LogFile -LogString "Subnet $ScopeName already exists" -ForegroundColor Green
                        }
                        default {
