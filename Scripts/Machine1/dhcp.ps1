...
$Site = @("Site1", "Site2")
$FeatureName = @("rsat-ad-powershell", "rsat-adds", "GPMC", "routing", "dhcp", "RSAT-DFS-Mgmt-Con", "rsat-dns-server", "rsat-dhcp", "WDS-AdminPack", "RSAT-ADCS", "UpdateServices-RSAT")
...
if ((gwmi win32_computersystem).partofdomain -eq $false) {
    ...
    if ((gwmi win32_computersystem).partofdomain -eq $true) {
        ...
        try {
            ...
            Write-Host "$Domain\ADM_Task_DHCP_Admins is already a member of DHCP Administrators" -ForegroundColor Green
        }
        try {
            Add-LocalGroupMember -Group "DHCP Users" -Member "$Domain\ADM_Task_DHCP_Users" -ErrorAction Stop
        } catch [Microsoft.PowerShell.Commands.MemberExistsException] {
            Write-Host "$Domain\ADM_Task_DHCP_Users is already a member of DHCP Users" -ForegroundColor Green
        }
