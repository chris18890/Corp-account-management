...
Create-ADGroup -GroupName $StaffGroup -Path "OU=Groups,$EndPath" -GroupDescription "Org-wide group for all users"
Create-ADGroup -GroupName $ITGroup -Path "OU=Groups,$EndPath" -GroupDescription "Departmental group holding all IT accounts"
Create-ADOU -OUName $ITGroup -Path $EndPath -OUDescription "Top level OU for IT User & Group objects"
Create-ADOU -OUName "Shared_Mailbox_Access" -Path "OU=Groups,$EndPath" -OUDescription "IT OU for Group objects that grant access to shared mailboxes"
Create-ADOU -OUName "Equipment_Mailbox_Access" -Path "OU=Groups,$EndPath" -OUDescription "IT OU for Group objects that grant access to equipment mailboxes"
Create-ADOU -OUName "Room_Mailbox_Access" -Path "OU=Groups,$EndPath" -OUDescription "IT OU for Group objects that grant access to room mailboxes"
...
Add-GroupMember -group "RG_Password_Admins" -Member "UG_Level_1_Admins"
...
if (!(TEST-PATH "\\$DNSSuffix\$ShareName")) {
    ...
    New-DfsnRoot -TargetPath "\\$ServerName\$ShareName" -Type DomainV2 -Path "\\$DNSSuffix\$ShareName" -GrantAdminAccounts "DFS_Admins"
...
if (!(TEST-PATH "\\$DNSSuffix\$ShareName")) {
    ...
    New-DfsnRoot -TargetPath "\\$ServerName\$ShareName" -Type DomainV2 -Path "\\$DNSSuffix\$ShareName" -GrantAdminAccounts "DFS_Admins"
    New-DfsReplicationGroup -GroupName "Profiles" | New-DfsReplicatedFolder -FolderName "Profiles" -DfsnPath "\\$DNSSuffix\Profiles" | Add-DfsrMember -ComputerName "$ServerName"
    Set-DfsrMembership -GroupName "Profiles" -FolderName "Profiles" -ContentPath "$Drive\Profiles" -ComputerName "$ServerName" -PrimaryMember $True -StagingPathQuotaInMB 16384 -Force
    Grant-DfsrDelegation -GroupName "Profiles" -AccountName "RG_DFS_Admins"
} else {
    Write-Host "\\$DNSSuffix\$ShareName already exists" -ForegroundColor Green
}
#====================================================================
