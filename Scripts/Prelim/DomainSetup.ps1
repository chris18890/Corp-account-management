...
function New-ADOU {
    ...
    Write-Log -LogFile $LogFile -LogString "Creating OU $OUName"
    try {
        ...
        Write-Log -LogFile $LogFile -LogString "Created OU $OUName"
    } catch {
        $ex = $_.Exception
        if ($ex.Message -match "already in use") {
            Write-Log -LogFile $LogFile -LogString "'$OUName' already exists" -ForegroundColor Green
        } else {
            throw
        }
    }
...
function Add-GPOLink {
    ...
    # Notes:
    #================================================================
    Write-Log -LogFile $LogFile -LogString "Linking $GPOName to $GPOTarget"
    try {
        New-GPLink -name $GPOName -target $GPOTarget -LinkEnabled Yes -enforced yes -Order 1 -ErrorAction Stop -Server $DCHostName
        Write-Log -LogFile $LogFile -LogString "Linked $GPOName to $GPOTarget"
    } catch {
        $ex = $_.Exception
        if ($ex.Message -match "already exists") {
            Write-Log -LogFile $LogFile -LogString "'$GPOName' already linked to $GPOTarget" -ForegroundColor Green
        } else {
            throw
        }
    }
...
#====================================================================
Write-Log -LogFile $LogFile -LogString "Adding $EmailSuffix as an additional UPN suffix"
$forest = Get-ADForest
...
#====================================================================
Write-Log -LogFile $LogFile -LogString "Preventing standard users from creating computer accounts"
...
#====================================================================
Write-Log -LogFile $LogFile -LogString "Enabling PAM feature to use temporal group membership"
...
#====================================================================
Write-Log -LogFile $LogFile -LogString "Enabling AD Recycle Bin"
...
#====================================================================
Write-Log -LogFile $LogFile -LogString "Protecting Domain Controllers OU"
Set-ADOrganizationalUnit -Identity "OU=Domain Controllers,$EndPath" -ProtectedFromAccidentalDeletion $true -Server $DCHostName
#====================================================================

Write-Log -LogFile $LogFile -LogString "Creating user OUs & Groups"
#====================================================================
# Staff OU & group creation
#====================================================================
New-ADOU -LogFile $LogFile -OUName $StaffGroup -Path $EndPath -OUDescription "Top level OU for Staff User objects"
New-ADOU -LogFile $LogFile -OUName $GroupsOU -Path $EndPath -OUDescription "Top level OU for Group objects"
...
New-ADOU -LogFile $LogFile -OUName "Administration" -Path $EndPath -OUDescription "Top level OU for IT Admin User & Group objects"
New-ADOU -LogFile $LogFile -OUName "Equipment_Mailbox_Access" -Path "OU=$GroupsOU,$EndPath" -OUDescription "IT Admin OU for Group objects that grant access to equipment mailboxes"
New-ADOU -LogFile $LogFile -OUName "Room_Mailbox_Access" -Path "OU=$GroupsOU,$EndPath" -OUDescription "IT Admin OU for Group objects that grant access to room mailboxes"
New-ADOU -LogFile $LogFile -OUName "Shared_Mailbox_Access" -Path "OU=$GroupsOU,$EndPath" -OUDescription "IT Admin OU for Group objects that grant access to shared mailboxes"
New-ADOU -LogFile $LogFile -OUName "Equipment_Mailbox_Accounts" -Path "OU=$AdministrationOU,$EndPath" -OUDescription "IT Admin OU for User objects that are equipment mailbox recipient types"
New-ADOU -LogFile $LogFile -OUName "Hi_Priv_Groups" -Path "OU=$AdministrationOU,$EndPath" -OUDescription "IT Admin OU for Group objects that control Hi-Priv access"
New-ADOU -LogFile $LogFile -OUName "Hi_Priv_Accounts" -Path "OU=$AdministrationOU,$EndPath" -OUDescription "IT Admin OU for User objects that are Hi-Priv accounts"
New-ADOU -LogFile $LogFile -OUName "Local_Admin_Groups" -Path "OU=$AdministrationOU,$EndPath" -OUDescription "IT Admin OU for Group objects that give local admin on individual devices"
New-ADOU -LogFile $LogFile -OUName "Room_Mailbox_Accounts" -Path "OU=$AdministrationOU,$EndPath" -OUDescription "IT Admin OU for User objects that are room mailbox recipient types"
New-ADOU -LogFile $LogFile -OUName "Shared_Mailbox_Accounts" -Path "OU=$AdministrationOU,$EndPath" -OUDescription "IT Admin OU for User objects that are shared mailbox recipient types"
New-ADOU -LogFile $LogFile -OUName "Service_Accounts" -Path "OU=$AdministrationOU,$EndPath" -OUDescription "IT Admin OU for User objects that are service accounts"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $ITAdminGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Group holding all IT Admin accounts"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_AD_Administration_OU_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members have Full Control over AD Site objects"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_AD_Computer_OU_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members have Full Control over AD Site objects"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_AD_GPO_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members can create & link GPOs"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_AD_Group_OU_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members have Full Control over AD Site objects"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_AD_Site_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members have Full Control over AD Site objects"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_AD_Subnet_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members have Full Control over AD Subnet objects"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_AD_Transport_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members have Full Control over AD Transport objects"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_AD_User_OU_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members have Full Control over AD Site objects"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_Desktop_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members are added to Local Admin on all computers in the Desktop, Laptop, & VM OUs"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_DFS_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members have Full Control NTFS permissions on all DFS folders & have access to DFS console"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_DHCP_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members are members of DHCP Administrators"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_DHCP_Users" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members are members of DHCP Users"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $DNSOperatorsGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members have Edit access to DNS zones"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $DNSReadOnlyGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members have read only access to the DNS service"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $HiPrivAccountAdminGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members can create/delete/edit accounts in the Hi_Priv_Accounts OU"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $HiPrivGroupAdminGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members can create/delete/edit groups in the Hi_Priv_Groups OU"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $InstallerGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members have permission to join & move computer objects in $ParentOU & Sub OUs"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $LocalAdminGroupAdminGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members can create/delete/edit groups in the Local_Admin_Groups OU"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $UserPasswordDelegationGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members can reset passwords of users in the $StaffGroup OU"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_Server_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members are added to Local admin on all computers in $ParentOU & Sub OUs via GPO, are indirect members of the Server Operators BuiltIn group"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $SERAccessAdminGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members can edit the membership of sh_, eq_, & ro_ groups"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $SERAccountAdminGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members can create/delete/edit sh_, eq_, & ro_ accounts"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $ServiceAccountAdminGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members can create/delete/edit accounts in the Service_Accounts OU"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $StandardAccountAdminGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members can create/delete/edit accounts in the $StaffGroup OU"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName $StandardGroupAdminGroup -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members can create/delete/edit groups in the $GroupsOU OU"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_WDS_Deploy_Clients" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members can use WDS to deploy images in the Clients folder"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Task_WDS_Deploy_Servers" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Members can use WDS to deploy images in the Servers folder"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Role_Tier1_Level_1_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Level 1 admins - desktop"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Role_Tier1_Level_2_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Level 2 admins - junior server (Tier 1)"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Role_Tier1_Level_3_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Level 3 admins - senior server (Tier 1)"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Role_Tier0_Level_2_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Level 2 admins - junior server (Tier 0)"
New-DomainGroup -LogFile $LogFile -DCHostName $DCHostName -GroupName "ADM_Role_Tier0_Level_3_Admins" -GroupCategory $GroupCategory -GroupScope $GroupScope -O365 "N" -HiddenFromAddressListsEnabled $true -Path "OU=Hi_Priv_Groups,OU=$AdministrationOU,$EndPath" -GroupDescription "Level 3 admins - senior server (Tier 0)"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $StaffGroup -Member $ITGroup
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Remote Desktop Users" -Member "ADM_Role_Tier0_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Server Operators" -Member "ADM_Role_Tier0_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Remote Desktop Users" -Member "ADM_Role_Tier0_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Server Operators" -Member "ADM_Role_Tier0_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $ITAdminGroup -Member $SID500
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_AD_Administration_OU_Admins" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_AD_Computer_OU_Admins" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_AD_GPO_Admins" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_AD_Group_OU_Admins" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_AD_Site_Admins" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_AD_Subnet_Admins" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_AD_Transport_Admins" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_AD_User_OU_Admins" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_Desktop_Admins" -Member "ADM_Role_Tier1_Level_1_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_Desktop_Admins" -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_Desktop_Admins" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_DHCP_Admins" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_DHCP_Users" -Member "ADM_Role_Tier1_Level_1_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_DHCP_Users" -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_DFS_Admins" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $DNSOperatorsGroup -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $DNSOperatorsGroup -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $DNSReadOnlyGroup -Member "ADM_Role_Tier1_Level_1_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $HiPrivAccountAdminGroup -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $HiPrivGroupAdminGroup -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $InstallerGroup -Member "ADM_Role_Tier1_Level_1_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $InstallerGroup -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $InstallerGroup -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $LocalAdminGroupAdminGroup -Member "ADM_Role_Tier1_Level_1_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $LocalAdminGroupAdminGroup -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $LocalAdminGroupAdminGroup -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $UserPasswordDelegationGroup -Member "ADM_Role_Tier1_Level_1_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $UserPasswordDelegationGroup -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $UserPasswordDelegationGroup -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_Server_Admins" -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_Server_Admins" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $SERAccessAdminGroup -Member "ADM_Role_Tier1_Level_1_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $SERAccessAdminGroup -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $SERAccessAdminGroup -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $SERAccountAdminGroup -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $SERAccountAdminGroup -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $StandardAccountAdminGroup -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $StandardAccountAdminGroup -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $StandardGroupAdminGroup -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $StandardGroupAdminGroup -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $ServiceAccountAdminGroup -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group $ServiceAccountAdminGroup -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_WDS_Deploy_Clients" -Member "ADM_Role_Tier1_Level_1_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_WDS_Deploy_Clients" -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_WDS_Deploy_Clients" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_WDS_Deploy_Servers" -Member "ADM_Role_Tier1_Level_2_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "ADM_Task_WDS_Deploy_Servers" -Member "ADM_Role_Tier1_Level_3_Admins"
#====================================================================

Write-Log -LogFile $LogFile -LogString "Creating shares"
#====================================================================
# Create main Share
#====================================================================
$ShareName = $RootShare
if (!(TEST-PATH "\\$DNSSuffix\$ShareName")) {
    if (!(TEST-PATH "\\$ServerName\$ShareName")) {
        if (!(TEST-PATH "$Drive\$ShareName")) {
            ...
            Write-Log -LogFile $LogFile -LogString "$Drive\$ShareName already exists" -ForegroundColor Green
        }
        New-SmbShare -Name $ShareName -Path "$Drive\$ShareName" -FullAccess "Administrators", "SYSTEM" -ChangeAccess "authenticated users"
        Write-Log -LogFile $LogFile -LogString "Pausing for 60 seconds after creating share $ShareName"
        Start-Sleep -s 60
    } else {
        Write-Log -LogFile $LogFile -LogString "\\$ServerName\$ShareName already exists" -ForegroundColor Green
    }
    ...
    Write-Log -LogFile $LogFile -LogString "\\$DNSSuffix\$ShareName already exists" -ForegroundColor Green
}
#====================================================================
...
if (!(TEST-PATH "\\$DNSSuffix\$RootShare\$ShareName")) {
    ...
    Write-Log -LogFile $LogFile -LogString "\\$DNSSuffix\$RootShare\$ShareName already exists" -ForegroundColor Green
}
$ShareName = "Software"
if (!(TEST-PATH "\\$DNSSuffix\$RootShare\$ShareName")) {
    New-Item "\\$DNSSuffix\$RootShare\$ShareName" -type directory
...
Write-Log -LogFile $LogFile -LogString "Creating computer OUs"
#====================================================================
# Create default computers OU
#====================================================================
New-ADOU -LogFile $LogFile -OUName $ParentOU -Path $EndPath -OUDescription "Top level OU for Computer objects"
#====================================================================

#====================================================================
# Create OU for servers
#====================================================================
New-ADOU -LogFile $LogFile -OUName "Servers" -Path $Location
#====================================================================

#====================================================================
# Create OU for desktops
#====================================================================
New-ADOU -LogFile $LogFile -OUName "Desktops" -Path $Location
#====================================================================

#====================================================================
# Create OU for laptops
#====================================================================
New-ADOU -LogFile $LogFile -OUName "Laptops" -Path $Location
#====================================================================

#====================================================================
# Create OU for VMs
#====================================================================
New-ADOU -LogFile $LogFile -OUName "VMs" -Path $Location
#====================================================================

#====================================================================
# Redirect default computer location & delegate permissions
#====================================================================
Write-Log -LogFile $LogFile -LogString "Creating Permission delegations"
$guidMap = @{}
$GuidMapParams = @{
...
# Set up LAPS
#====================================================================
Write-Log -LogFile $LogFile -LogString "Setting up LAPS"
...
#====================================================================
Write-Log -LogFile $LogFile -LogString "Securing built-in admin account"
...
# Import GPOs
#====================================================================
Write-Log -LogFile $LogFile -LogString "Creating & linking GPOs"
$GPOName = "Default Domain Policy"
try {
    Set-GPLink -name $GPOName -target $EndPath -enforced yes -ErrorAction Stop -Server $DCHostName
    Write-Log -LogFile $LogFile -LogString "Enforced $GPOName on $EndPath"
} catch {
    $ex = $_.Exception
    throw
}
...
Add-GPOLink -LogFile $LogFile -GPOName "Logon Policy" -GPOTarget $EndPath
$ImportedGPOs += Import-GPO -BackupGpoName "TLS" -TargetName "TLS" -path $GPOLocation -CreateIfNeeded -Server $DCHostName
Add-GPOLink -LogFile $LogFile -GPOName "TLS" -GPOTarget $EndPath
...
Add-GPOLink -LogFile $LogFile -GPOName "CWDIllegalInDllSearch" -GPOTarget $EndPath
...
Add-GPOLink -LogFile $LogFile -GPOName "DisableNullSessionEnumeration" -GPOTarget $EndPath
...
Add-GPOLink -LogFile $LogFile -GPOName "EnableSMBSigning" -GPOTarget $EndPath
...
Add-GPOLink -LogFile $LogFile -GPOName "EnforceNLAandTLSforRDP" -GPOTarget $EndPath
...
Add-GPOLink -LogFile $LogFile -GPOName "GroupPolicyHardening_MS150-011" -GPOTarget $EndPath
...
Add-GPOLink -LogFile $LogFile -GPOName "Kerberos_Armouring" -GPOTarget $EndPath
...
Add-GPOLink -LogFile $LogFile -GPOName "LSAProtection" -GPOTarget $EndPath
$ImportedGPOs += Import-GPO -BackupGpoName "NTLMv2" -TargetName "NTLMv2" -path $GPOLocation -CreateIfNeeded -Server $DCHostName
Add-GPOLink -LogFile $LogFile -GPOName "NTLMv2" -GPOTarget $EndPath
...
Add-GPOLink -LogFile $LogFile -GPOName "LDAP_signing_requirements" -GPOTarget $EndPath
...
Add-GPOLink -LogFile $LogFile -GPOName "DC_AppLocker_Disable_Browsers" -GPOTarget "ou=Domain Controllers,$EndPath"
...
Add-GPOLink -LogFile $LogFile -GPOName "DC_Auditing" -GPOTarget "ou=Domain Controllers,$EndPath"
...
Add-GPOLink -LogFile $LogFile -GPOName "DC_Disable_Print_Spooler" -GPOTarget "ou=Domain Controllers,$EndPath"
...
Add-GPOLink -LogFile $LogFile -GPOName "DC_LDAP_signing_requirements" -GPOTarget "ou=Domain Controllers,$EndPath"
...
Add-GPOLink -LogFile $LogFile -GPOName "ADM_Task_Server_Admins as members of Local admins" -GPOTarget $Location
...
Add-GPOLink -LogFile $LogFile -GPOName "LAPS" -GPOTarget $Location
...
Add-GPOLink -LogFile $LogFile -GPOName "ADM_Task_Desktop_Admins as members of Local admins" -GPOTarget "OU=Desktops,$Location"
Add-GPOLink -LogFile $LogFile -GPOName "ADM_Task_Desktop_Admins as members of Local admins" -GPOTarget "OU=Laptops,$Location"
Add-GPOLink -LogFile $LogFile -GPOName "ADM_Task_Desktop_Admins as members of Local admins" -GPOTarget "OU=VMs,$Location"
...
Add-GPOLink -LogFile $LogFile -GPOName "IT Desktop Prefs" -GPOTarget "OU=$StaffGroup,$EndPath"
Add-GPOLink -LogFile $LogFile -GPOName "IT Desktop Prefs" -GPOTarget "OU=Hi_Priv_Accounts,OU=$AdministrationOU,$EndPath"
try {
    Write-Log -LogFile $LogFile -LogString "Updating permissions on IT Desktop Prefs"
    ...
    Write-Log -LogFile $LogFile -LogString "Updated permissions on IT Desktop Prefs"
} catch {
    throw
}
...
Add-GPOLink -LogFile $LogFile -GPOName "CM visual help" -GPOTarget "OU=$StaffGroup,$EndPath"
Add-GPOLink -LogFile $LogFile -GPOName "CM visual help" -GPOTarget "OU=Hi_Priv_Accounts,OU=$AdministrationOU,$EndPath"
try {
    Write-Log -LogFile $LogFile -LogString "Updating permissions on CM visual help"
    ...
    Write-Log -LogFile $LogFile -LogString "Updated permissions on CM visual help"
} catch {
    throw
}
foreach ($GPOName in $ImportedGPOs) {
    try {
        Write-Log -LogFile $LogFile -LogString "Updating permissions on $GPOName.DisplayName"
        ...
        Write-Log -LogFile $LogFile -LogString "Updated permissions on $GPOName.DisplayName"
    } catch {
        throw
    }
}
