...
Create-ADGroup -GroupName "ADM_Task_DHCP_Users" -Path "OU=Hi_Priv_Groups,OU=$ITGroup,$EndPath" -GroupDescription "Members are members of DHCP Users"
...
Create-ADGroup -GroupName "ADM_Task_DNS_ReadOnly" -Path "OU=Hi_Priv_Groups,OU=$ITGroup,$EndPath" -GroupDescription "Members have read only access to the DNS service"
...
Create-ADGroup -GroupName "ADM_Task_Subscription_Owners" -Path "OU=Hi_Priv_Groups,OU=$ITGroup,$EndPath" -GroupDescription "Members have Owner permissions on the subscription"
Create-ADGroup -GroupName "ADM_Task_Subscription_User_Access_Admins" -Path "OU=Hi_Priv_Groups,OU=$ITGroup,$EndPath" -GroupDescription "Members have User Access Admin permissions on the subscription"
Create-ADGroup -GroupName "ADM_Task_WDS_Deploy_Clients" -Path "OU=Hi_Priv_Groups,OU=$ITGroup,$EndPath" -GroupDescription "Members can use WDS to deploy images in the Clients folder"
Create-ADGroup -GroupName "ADM_Task_WDS_Deploy_Servers" -Path "OU=Hi_Priv_Groups,OU=$ITGroup,$EndPath" -GroupDescription "Members can use WDS to deploy images in the Servers folder"
Create-ADGroup -GroupName "ADM_Role_Level_1_Admins" -Path "OU=Hi_Priv_Groups,OU=$ITGroup,$EndPath" -GroupDescription "Level 1 admins - desktop, includes all level 2 admins"
Create-ADGroup -GroupName "ADM_Role_Level_2_Admins" -Path "OU=Hi_Priv_Groups,OU=$ITGroup,$EndPath" -GroupDescription "Level 2 admins - junior server, includes all level 3 admins"
Create-ADGroup -GroupName "ADM_Role_Level_3_Admins" -Path "OU=Hi_Priv_Groups,OU=$ITGroup,$EndPath" -GroupDescription "Level 3 admins - senior server"
...
Add-GroupMember -group "ADM_Task_DNS_ReadOnly" -Member "ADM_Role_Level_1_Admins"
Add-GroupMember -group "ADM_Task_DNS_ReadOnly" -Member "ADM_Role_Level_2_Admins"
...
Add-GroupMember -group "ADM_Role_Level_1_Admins" -Member "ADM_Role_Level_2_Admins"
Add-GroupMember -group "ADM_Role_Level_2_Admins" -Member "ADM_Role_Level_3_Admins"
...
if (!(TEST-PATH "\\$DNSSuffix\$ShareName")) {
    ...
    Grant-DfsrDelegation -GroupName "$ShareName" -AccountName "ADM_Task_DFS_Admins"
} else {
