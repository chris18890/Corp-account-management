...
Write-Log -LogFile $LogFile -LogString "Total Local Admin groups before cleaning: $($DomainLocalList.count)"
foreach ($c in $DomainLocalList) {
    ...
    try {
        ...
        Write-Log -LogFile $LogFile -LogString "Group $($c.Name) deleted"
    }
}

#Get a list of all Local Admin Groups and report on the number after cleaning
$DomainLocalList = Get-ADGroup -Filter {name -like "ADM_Task_Local_Admin_*"} -SearchBase $GroupsOU -Server $DCHostName | select name
Write-Log -LogFile $LogFile -LogString "Total Local Admin groups after cleaning: $($DomainLocalList.count)"
