...
$ScriptTitle = "$Domain Intermediate CA Config Script"
...
if (!$LogFile) {
    $LogFileName = $Domain + "_intermediate_CA_config_log-$(Get-Date -Format 'yyyyMMdd')"
    $LogIndex = 0
...
$Drive = $Drive.TrimEnd(':') + ':'
$ShareName = $Env.Shares.CertEnroll
$FeatureName = @("Adcs-Cert-Authority", "ADCS-Web-Enrollment", "rsat-ad-powershell", "rsat-dns-server")
Foreach ($Feature in $FeatureName){
    if (Get-WindowsFeature -Name $Feature | Where-Object InstallState -Eq Installed) {
        Write-LogFile -LogFile $LogFile -LogString "$Feature is already installed"
    } else {
...
if ((Get-CimInstance win32_computersystem).partofdomain -eq $false) {
    ...
    if ((Get-CimInstance win32_computersystem).partofdomain -eq $true) {
        ...
        if (-not $groups) {
            Write-LogFile -LogFile $LogFile -LogString "Invoker is not authorised to run this script. Required privileges not present. Aborting."
...
$CAPolicyInf | Out-File "C:\Windows\CAPolicy.inf" -Encoding utf8 -Force
        ...
        if (!(TEST-PATH "\\$ServerName\$ShareName")) {
            if (!(TEST-PATH "$Drive\$ShareName")) {
                ...
                Write-LogFile -LogFile $LogFile -LogString "$Drive\$ShareName already exists"
            }
            if (-not (Get-SmbShare -Name $ShareName -ErrorAction SilentlyContinue)) {
                ...
                Write-LogFile -LogFile $LogFile -LogString "SMB share '$ShareName' already exists"
            }
        } else {
            Write-LogFile -LogFile $LogFile -LogString "\\$ServerName\$ShareName already exists"
        }
        if (-not (Get-WebVirtualDirectory -Site "Default Web Site" -Name $ShareName -ErrorAction SilentlyContinue)) {
            ...
            Write-LogFile -LogFile $LogFile -LogString "Virtual directory '$ShareName' already exists"
        }
        if (-not (Get-WebBinding -Name "Default Web Site" -HostHeader $httpCRLPath -ErrorAction SilentlyContinue)) {
            ...
            Write-LogFile -LogFile $LogFile -LogString "Web binding for '$httpCRLPath' already exists"
        }
        
        Push-Location -Path "$env:SystemRoot\system32\inetsrv"
        try {
        ...
        if (-not (Get-DnsServerResourceRecord -ComputerName $DCHostName -ZoneName $DNSSuffix -Name "ca" -ErrorAction SilentlyContinue)) {
            ...
            Write-LogFile -LogFile $LogFile -LogString "DNS CNAME 'ca' already exists"
        }
