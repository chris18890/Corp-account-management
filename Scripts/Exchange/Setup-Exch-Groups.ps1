...
$MailGroups = (Get-ADGroup -Filter * -Server $DCHostName -searchbase "OU=Groups,$EndPath" -Properties *).Name
...
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Organization Management" -Member "ADM_Role_Tier1_Level_3_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Server Management" -Member "ADM_Task_Server_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Recipient Management" -Member "ADM_Task_HiPriv_Account_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Recipient Management" -Member "ADM_Task_HiPriv_Group_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Recipient Management" -Member "ADM_Task_Standard_Account_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Recipient Management" -Member "ADM_Task_Standard_Group_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Recipient Management" -Member "ADM_Task_SER_Account_Admins"
Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Help Desk" -Member "ADM_Role_Tier1_Level_1_Admins"
...
if (-not (Get-WebBinding -Name "Default Web Site" -HostHeader "$subdomain.$EmailSuffix" -ErrorAction SilentlyContinue)) {
    ...
    Write-Log -LogFile $LogFile -LogString "Web binding for '$subdomain.$EmailSuffix' already exists" -ForegroundColor Green
}

Write-Log -LogFile $LogFile -LogString "Setting Virtual Directories"
...
Write-Log -LogFile $LogFile -LogString "Enabling Download Domains"
...
Write-Log -LogFile $LogFile -LogString "Setting accepted domains and address policies"
# add accepted domain
