...
if ([string]::IsNullOrWhiteSpace($UserNameGlobalAdmin) -or $UserNameGlobalAdmin -eq 'ga.') {
    Write-LogFile -LogFile $LogFile -LogString "ERROR: Global admin username became invalid after sanitisation. Skipping." -ForegroundColor Red
    return
}
if (!$Manager) {
