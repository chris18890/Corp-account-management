...
if ([string]::IsNullOrWhiteSpace($UserNameCloudAdmin) -or $UserNameCloudAdmin -eq 'ca.') {
    Write-LogFile -LogFile $LogFile -LogString "ERROR: Cloud admin username became invalid after sanitisation. Skipping." -ForegroundColor Red
    return
}
if (!$Manager) {
