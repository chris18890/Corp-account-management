...
if ([string]::IsNullOrWhiteSpace($UserNames)) {
    Write-Log "You pressed the Cancel button, exiting."
    Break
}
...
foreach ($UserName in $UserNames -split '\s+') {
    ...
    Write-Log ""
    Write-Log "-----------------------"
    Write-Log "Account $UserName deleted at $t on $d by $u"
    Write-Log "-----------------------"
    #################
    Write-Log "Processing $UserName"# load the account from the AD
    Write-Log "-----------------------"
    $user = Get-ADUser -Filter "sAMAccountName -eq '$UserName'" -Properties * -Server $DCHostName
    #$user
    # check it was found
    if (!$user) {
        Write-Log " > ID $UserName not found"
        Write-Log "-----------------------"
        continue # skip to the next ID
    }
    Write-Log (" > This account was for " + $user.displayname)
    #################
    Write-Log " - Ensuring $UserName disabled"
    if ($user.enabled -eq $true) {
        Write-Log "   - Account $UserName is not disabled!"
        if (Show-MessageBox $ScriptTitle "Account $UserName is not disabled. Are you sure you want to continue?") {
            Write-Log "   > Proceeding at user request, disabling the account $UserName"
            Set-ADUser -Identity $user -Enabled $false -Server $DCHostName
        } else {
            Write-Log "Skipping $UserName at user request"
            continue # skip to the next ID
        }
    }
    #################
    Write-Log " - Doing other things"
    #################
    Write-Log " - Removing Home Drive"
    $HomeDrive = $user.HomeDirectory
    if ($HomeDrive) {
        Write-Log "   > $HomeDrive"
        if (Test-Path $HomeDrive) {
            Write-Log "   - HomeDrive $HomeDrive for $UserName exists"
            if (Show-MessageBox $ScriptTitle "HomeDrive $HomeDrive for $UserName exists. Are you sure you want to continue?") {
                Write-Log "   > Proceeding at user request, deleting home directory $HomeDrive for $UserName"
                Remove-Item -Recurse -Force $HomeDrive
            } else {
                Write-Log "Skipping deletion of $HomeDrive for $UserName at user request"
            }
        }
        Write-Log " - Home Drive was at $HomeDrive"
    } else {
        Write-Log " - No Home Drive configured"
    }
    ...
    try {
        ...
        Write-Log " - Removing group $SharedGroupName for shared account $UserName"
    } catch {
        Write-Log " - Note: $($_.Exception.Message)" -ForegroundColor Yellow
    }
    ...
    try {
        ...
        Write-Log " - Removing group $EquipmentGroupName for equipment account $UserName"
    } catch {
        Write-Log " - Note: $($_.Exception.Message)" -ForegroundColor Yellow
    }
    ...
    try {
        ...
        Write-Log " - Removing group $RoomGroupName for room account $UserName"
    } catch {
        Write-Log " - Note: $($_.Exception.Message)" -ForegroundColor Yellow
    }
    try {
        ...
        Write-Log " - Deleting account $UserName"
    } catch {
        Write-Log " - Note: $($_.Exception.Message)" -ForegroundColor Yellow
    }
    Write-Log "-----------------------"
