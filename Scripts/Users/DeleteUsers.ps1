...
# Ask for some work
$UserNames = Read-MultiLineInputBoxDialog -Message "Type or paste in a list of IDs to delete" -WindowTitle $ScriptTitle ""
...
foreach ($entry in $UserNames -split '\s+') {
    ...
    # Detect whether the caller already supplied a prefixed account name.
    # Accepted prefixes: admin.  da.  ca.  ga.
    # ca.*/ga.* are cloud-only - they go into the O365 deletion branch,
    # not the on-prem branch, so we skip them here.
    ...
    if ($isCloud) {
        ...
        # Bare username - expand to all three on-prem tiers.
        $onPremAccounts.Add($entry)
...
foreach ($UserName in $onPremAccounts) {
    ...
    if (-not (Show-MessageBox $ScriptTitle "Permanently delete account $UserName ($($user.displayname))? It is currently $state. This removes the home drive, access groups and the AD account, and cannot be undone.")) {
        Write-LogFile -LogFile $LogFile -LogString "Skipping $UserName at user request"
        continue # skip to the next ID
    }
    #################
    ...
    if ($HomeDrive) {
        ...
        if (Test-Path $HomeDrive) {
            ...
            if (Show-MessageBox $ScriptTitle "HomeDrive $HomeDrive for $UserName exists. Are you sure you want to continue?") {
                Write-LogFile -LogFile $LogFile -LogString "   > Proceeding at user request, deleting home directory $HomeDrive for $UserName"
                Remove-Item -Recurse -Force $HomeDrive
            } else {
                Write-LogFile -LogFile $LogFile -LogString "Skipping deletion of $HomeDrive for $UserName at user request"
            }
        }
...
if ($O365) {
    ...
    if ($Connected -eq $true) {
        ...
        foreach ($entry in $UserNames -split '\s+') {
            ...
            # Detect whether the caller already supplied a prefixed account name.
            # Accepted prefixes: admin.  da.  ca.  ga.
            # admin.*/da.* are on-prem-only - they were handled by the on-prem
            # deletion branch above, so we skip them here. Without this skip a
            # pasted 'admin.foo' would generate 'ca.admin.foo'/'ga.admin.foo'
            # Entra lookups, which are guaranteed not-found and pollute the
            # audit trail.
            $isCa     = $entry -match '^ca\.'
            $isGa     = $entry -match '^ga\.'
            ...
            if ($isOnPrem) {
                ...
                # Bare username - expand to both cloud tiers.
        ...
        foreach ($CloudUserPrincipalName in $CloudAccounts) {
            ...
            if (-not (Show-MessageBox $ScriptTitle "Permanently delete cloud account $CloudUserPrincipalName and remove its role assignments? This cannot be undone.")) {
                Write-LogFile -LogFile $LogFile -LogString "Skipping $CloudUserPrincipalName at user request"
                continue
            }
