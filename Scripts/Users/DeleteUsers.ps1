...
foreach ($UserName in $UserNames -split '\s+') {
    ...
    try {
        Set-ADObject -Identity $SharedGroupName -protectedFromAccidentalDeletion $False
    ...
    try {
        Set-ADObject -Identity $EquipmentGroupName -protectedFromAccidentalDeletion $False
    ...
    try {
        Set-ADObject -Identity $RoomGroupName -protectedFromAccidentalDeletion $False
    ...
    try {
        Set-ADObject -Identity $UserName -protectedFromAccidentalDeletion $False
