...
if ($ExistingUser) {
    ...
    Write-Log -LogFile $LogFile -LogString "ERROR: User '$($UserNameDomainAdmin)' already exists in the $Domain directory. The user object`n is '$($ExistingUser.DistinguishedName)'" -ForegroundColor Red
    Write-Log -LogFile $LogFile -LogString "Error: Processing '$LastName, $FirstName' ($UserNameDomainAdmin) aborted" -ForegroundColor Red
    Write-Log -LogFile $LogFile -LogString ("-" * 80) -ForegroundColor Red
    Write-Log -LogFile $LogFile -LogString " "
    ...
    try {
        Write-Log -LogFile $LogFile -LogString ("=" * 80)
        Write-Log -LogFile $LogFile -LogString "Processing '$DisplayName' ($UserNameDomainAdmin)..."
        Write-Log -LogFile $LogFile -LogString ("=" * 80)
        ...
        Write-Log -LogFile $LogFile -LogString "Creating $UserNameDomainAdmin"
        ...
        if ($PrivLevel -ge 3) {
            Add-GroupMember -LogFile $LogFile -DCHostName $DCHostName -Group "Domain Admins" -Member $UserNameDomainAdmin
        }
        ...
        Write-Log -LogFile $LogFile -LogString ("=" * 80)
        Write-Log -LogFile $LogFile -LogString "Processing for '$DisplayName' ($UserNameDomainAdmin) complete"
        Write-Log -LogFile $LogFile -LogString ("=" * 80)
        Write-Log -LogFile $LogFile -LogString " "
    } catch {
        $e = $_.Exception
        ...
        Write-Log -LogFile $LogFile -LogString ("-" * 80) -ForegroundColor Red
        Write-Log -LogFile $LogFile -LogString "ERROR: Failed during processing of '$DisplayName' ($UserNameDomainAdmin) - Line $Line" -ForegroundColor Red
