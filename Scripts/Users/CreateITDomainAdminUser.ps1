...
if ([string]::IsNullOrWhiteSpace($UserNameDomainAdmin) -or $UserNameDomainAdmin -eq 'da.') {
    Write-LogFile -LogFile $LogFile -LogString "ERROR: Domain admin username became invalid after sanitisation. Skipping." -ForegroundColor Red
    return
}
if (!$Manager) {
