...
if ([string]::IsNullOrWhiteSpace($UserNameAdmin) -or $UserNameAdmin -eq 'admin.') {
    Write-LogFile -LogFile $LogFile -LogString "ERROR: Admin username became invalid after sanitisation. Skipping." -ForegroundColor Red
    return
}
if (!$Manager) {
