...
if ($ExistingUser) {
    ...
    try {
        ...
        if ($PrivLevel -ge "3") {
            ...
            Write-Log ""
            .\CreateDomainAdminUser.ps1 -FirstName $FirstName -LastName $LastName -UserName $UserName -UserPassword $UserPassword -EmailSuffix $EmailSuffix -Dept $Dept -Company $Company -LogFile $LogFile -O365 $O365 -O365EmailSuffix $O365EmailSuffix -DCHostName $DCHostName -Manager $Manager
        }
        if ($O365 -eq "E") {
