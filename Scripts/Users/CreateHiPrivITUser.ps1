...
$PrivGroup = "UG_Level_" + $PrivLevel + "_Admins"
if (!$FirstName) {
...
if ($ExistingUser) {
    ...
    try {
        ...
        Set-ADAccountControl -AccountNotDelegated $false -AllowReversiblePasswordEncryption $false -CannotChangePassword $false -DoesNotRequirePreAuth $false -Identity "CN=$UserNameAdmin,$OUPath" -PasswordNeverExpires $false -UseDESKeyOnly $false -Server $DCHostName
        Add-GroupMember -Group $Membership -Member $UserNameAdmin
        ...
        if ($PrivLevel -ge "3") {
            ...
            Write-Log ""
            .\CreateDomainAdminUser.ps1 -FirstName $FirstName -LastName $LastName -UserName $UserName -UserPassword $UserPassword -EmailSuffix $EmailSuffix -Description $Description -Dept $Dept -Company $Company -LogFile $LogFile -O365 $O365 -O365EmailSuffix $O365EmailSuffix -DCHostName $DCHostName -Manager $Manager -Requester $Requester -SMTPServer $SMTPServer -EmailFrom $EmailFrom -PasswordLength $PasswordLength
        }
        if ($O365 -eq "E") {
            Write-Log "Exchange mailbox for $UserNameAdmin will be created in Exchange OnPrem"
            Write-Log "Calling Create-Mailbox-OnPrem function with the following parameters:"
            Write-Log "UserName: $UserNameAdmin"
            $enabledMailboxes += Create-Mailbox-OnPrem -UserName $UserNameAdmin
        ...
        if ($O365 -eq "H") {
            Write-Log "Exchange mailbox for $UserNameAdmin will be created in Exchange Online"
            Write-Log "Calling Create-Mailbox-Hybrid function with the following parameters:"
            Write-Log "UserName: $UserNameAdmin"
            $enabledMailboxes += Create-Mailbox-Hybrid -UserName $UserNameAdmin
            # Send email to requester and manager with new user's name & password
            Send-UserEmail -UserName $UserNameAdmin -password $Password -requester $Requester -manager $Manager
        }
        Write-Log ("=" * 80)
