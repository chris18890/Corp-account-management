...
if ($ExistingUser) {
    ...
    Write-Log "Error: Processing '$LastName, $FirstName' ($UserName) aborted" -ForegroundColor Red
    Write-Log ("-" * 80) -ForegroundColor Red
    Write-Log ""
    ...
    try {
        Write-Log ("=" * 80)
        Write-Log "Processing '$DisplayName' ($UserName)..."
        Write-Log ("=" * 80)
        $Params = @{
            Name                    = $UserName
        ...
        Add-GroupMember -Group $PrivGroup -Member $UserName
        Add-GroupMember -Group "Protected Users" -Member $UserName
        if ($PrivLevel -ge "3") {
            Write-Log "Creating Domain Admin account for $UserName"
            Write-Log ""
            .\CreateDomainAdminUser.ps1 -FirstName $FirstName -LastName $LastName -UserName $UserName -UserPassword $UserPassword -EmailSuffix $EmailSuffix -Dept $Dept -Company $Company -LogFile $LogFile -O365 $O365 -O365EmailSuffix $O365EmailSuffix -DCHostName $DCHostName -Manager $Manager -Requester $Requester
        }
        if ($O365 -eq "E") {
