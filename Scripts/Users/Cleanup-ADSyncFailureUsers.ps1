#Requires -Modules ActiveDirectory, Microsoft.Graph, ExchangeOnlineManagement
...
try {
    ...
    foreach ($USER in $UserList) {
        try {
            ...
            if ([string]::IsNullOrWhiteSpace($UserName)) {
                Write-LogFile -LogFile $LogFile -LogString "ERROR: USERNAME became empty after sanitisation. Skipping row." -ForegroundColor Red
                continue
            }
    ...
    if (Get-PSSession) {
        ...
        Get-PSSession | Where-Object {
            $_.ComputerName -eq $AzureADConnect -or $_.Name -like '*Exchange*'
        } | Remove-PSSession
