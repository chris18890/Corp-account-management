...
Describe 'CreateGroup.ps1 S/H branch parity' {
    ...
    Context 'H clause - Hi-Priv group, restricted OU, no Staff join' {
        ...
        It 'requires HiPriv_Group_Admins (precheck present)' {
...
# =============================================================================
# Invoker authorisation gating (parity with ChangePassword.ps1)
# =============================================================================
# CreateGroup.ps1 gates execution in two places, in the same shape as
# ChangePassword.ps1:
#   1. A top-level gate (BEFORE the GroupType switch) requires the invoker to be
#      a member of the Standard-tier group admins or Domain Admins. It applies to
#      EVERY run, regardless of GroupType.
#   2. An additional, stricter precheck INSIDE the H switch clause requires the
#      HiPriv-tier admin group (or Domain Admins) before a Hi-Priv group can be
#      created.
# The asymmetry is load-bearing: a Hi-Priv group must demand a strictly higher,
# privilege than creating a Staff group. Both gates log the refusal via Write-LogFile
# and then throw. Losing the top-level gate lets any caller through; losing the
# H-arm precheck is a privilege-escalation surface (a Standard_*_Admins member could
# create a HiPriv group). The S clause intentionally carries NO extra precheck - the
# top-level gate suffices for creating Staff groups, mirroring ChangePassword.ps1's
# S/H precheck asymmetry.
# =============================================================================
Describe 'CreateGroup.ps1 invoker authorisation gating' {
    BeforeAll {
        $cgPath = Join-Path $script:scriptsRoot 'Users\CreateGroup.ps1'
        $script:cgSource = Get-Content $cgPath -Raw
        $script:cgAst    = [System.Management.Automation.Language.Parser]::ParseFile(
            $cgPath, [ref]$null, [ref]$null
        )
        # Every Test-IsMemberOf invocation in the script.
        $script:cgMemberChecks = $script:cgAst.FindAll({
            param($n)
            $n -is [System.Management.Automation.Language.CommandAst] -and
            $n.GetCommandName() -eq 'Test-IsMemberOf'
        }, $true)
        # The GroupType switch and its clauses (keyed by clause value).
        $script:cgSwitch = $script:cgAst.FindAll({
            param($n) $n -is [System.Management.Automation.Language.SwitchStatementAst]
        }, $true) | Where-Object {
            $_.Condition.Extent.Text -match '\$GroupType'
        } | Select-Object -First 1
        ...
        # Text preceding the switch - the top-level gate lives here, so scoping
        # to it disambiguates the top-level requiredGroups / log+throw from the
        # H-arm copy further down.
        $script:cgPreSwitchText = if ($script:cgSwitch) {
            $script:cgSource.Substring(0, $script:cgSwitch.Extent.StartOffset)
        } else { '' }
        ...
        $script:cgTopLevelCheck = $script:cgMemberChecks | Where-Object {
            $script:cgSwitch -and
            $_.Extent.StartLineNumber -lt $script:cgSwitch.Extent.StartLineNumber
        } | Select-Object -First 1
        $script:cgHExtent = if ($script:cgClauses.ContainsKey('H')) {
            $script:cgClauses['H'].Extent
        } else { $null }
        $script:cgHArmCheck = $script:cgMemberChecks | Where-Object {
            $script:cgHExtent -and
            $_.Extent.StartLineNumber -ge $script:cgHExtent.StartLineNumber -and
            $_.Extent.EndLineNumber   -le $script:cgHExtent.EndLineNumber
        } | Select-Object -First 1
    }
    It 'gates the invoker with Test-IsMemberOf at least twice (top-level + H-arm)' {
        $script:cgMemberChecks.Count | Should -BeGreaterOrEqual 2
    }
    Context 'Top-level gate (applies to every GroupType)' {
        It 'runs BEFORE the GroupType switch' {
            $script:cgTopLevelCheck | Should -Not -BeNullOrEmpty
        }
        It 'passes -Sam $env:USERNAME, -GroupNames $requiredGroups and -DCHostName $DCHostName' {
            $t = $script:cgTopLevelCheck.Extent.Text
            $t | Should -Match '-Sam\s+\$env:USERNAME'
            $t | Should -Match '-GroupNames\s+\$requiredGroups'
            $t | Should -Match '-DCHostName\s+\$DCHostName'
        }
        It 'requires the Standard-tier group admins plus the additional Domain Admins' {
            ...
            $script:cgPreSwitchText | Should -Match '\$requiredGroups\s*=\s*@\(\s*"\$\(\$Env\.Groups\.TaskPrefix\)Standard_Group_Admins"\s*,\s*''Domain Admins''\s*\)'
        }
        It 'logs the refusal and throws when the invoker is not authorised' {
            $script:cgPreSwitchText | Should -Match '(?s)if\s*\(\s*-not\s*\(Test-IsMemberOf.*?Write-LogFile.*?not authorised.*?throw'
        }
    }
    Context 'H-arm precheck (privilege-escalation guard)' {
        It 'the H switch clause carries its OWN Test-IsMemberOf precheck' {
            $script:cgHArmCheck | Should -Not -BeNullOrEmpty
        }
        It 'the H precheck requires HiPriv_Group_Admins or Domain Admins' {
            $hBody = $script:cgClauses['H'].Extent.Text
            $hBody | Should -Match '\$requiredGroups\s*=\s*@\(\s*"\$\(\$Env\.Groups\.TaskPrefix\)HiPriv_Group_Admins"\s*,\s*''Domain Admins''\s*\)'
        }
        ...
        It 'demands a STRICTLY higher tier than the top-level gate (HiPriv, not Standard)' {
            # The whole point of the second gate: a Standard_Group_Admins member
            # must not be able to create a Hi-Priv group. The H-arm group token
            # must be the HiPriv tier and must NOT be the Standard tier.
            $hBody = $script:cgClauses['H'].Extent.Text
            $hBody | Should -Match 'HiPriv_\w+_Admins'
            $hBody | Should -Not -Match 'Standard_\w+_Admins'
        }
        It 'runs the precheck BEFORE creating the Hi-Priv group (fail closed)' {
            # The gate must throw before New-DomainGroup, so an unauthorised
            # caller never reaches the irreversible group creation.
            $hBody     = $script:cgClauses['H'].Extent.Text
            $gateIdx   = $hBody.IndexOf('Test-IsMemberOf')
            $createIdx = $hBody.IndexOf('New-DomainGroup')
            $gateIdx   | Should -BeGreaterOrEqual 0
            $createIdx | Should -BeGreaterOrEqual 0
            $gateIdx   | Should -BeLessThan $createIdx
        }
    }
    Context 'S clause carries no extra gate (top-level gate suffices for Staff groups)' {
        It 'the S switch clause does NOT contain its own Test-IsMemberOf precheck' {
            $sBody = $script:cgClauses['S'].Extent.Text
