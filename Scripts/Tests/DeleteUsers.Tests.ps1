...
# Coverage for the family's most destructive script. Input is multi-source
# (-InputCsv / -UserName / the Read-MultiLineInputBoxDialog paste box); per-account
# confirmation on the on-prem path is $PSCmdlet.ShouldProcess (the -Confirm pipeline).
# Behavioural tests would require live AD/Graph deletion or driving the GUI, so coverage
# is parameter-contract via Get-Command and source-pattern via the AST + raw text.
...
Describe 'DeleteUsers.ps1 parameter contract' {
    ...
    It 'exposes -RemoveHomeDirectory as a switch' {
        $script:cmd.Parameters['RemoveHomeDirectory'].SwitchParameter | Should -BeTrue
    }
    It 'exposes -NonInteractive as a switch' {
        $script:cmd.Parameters['NonInteractive'].SwitchParameter | Should -BeTrue
    }
    It 'exposes -LogFile, -EmailSuffix, -O365EmailSuffix, -UserName, InputCsv' {
        ...
        $script:cmd.Parameters.Keys | Should -Contain 'InputCsv'
    }
...
Describe 'DeleteUsers.ps1 destructive-action confirmation' {
    ...
    It 'gates EVERY deletion unconditionally via ShouldProcess, before (and independent of) the enabled check' {
        # Regression guard for the already-disabled bypass: the ShouldProcess
        # gate must precede the enabled handling, so an account that is already
        # disabled still requires confirmation.
        $script:source | Should -Match '(?s)\$PSCmdlet\.ShouldProcess[\s\S]*?\$user\.[Ee]nabled\s*-eq\s*\$true'
    }
...
Describe 'DeleteUsers.ps1 home-directory cleanup' {
    ...
    It 'gates Remove-Item behind -RemoveHomeDirectory and ShouldProcess' {
        $script:source | Should -Match '(?s)RemoveHomeDirectory[\s\S]*?ShouldProcess[\s\S]*?Remove-Item'
    }
}
...
Describe 'DeleteUsers.ps1 non-interactive operability' {
    BeforeAll { $script:source = Get-Content (Join-Path $script:scriptsRoot 'Users\DeleteUsers.ps1') -Raw }
    It 'declares ConfirmImpact High so silent runs require an explicit -Confirm:$false' {
        $script:source | Should -Match "ConfirmImpact\s*=\s*'High'"
    }
    It 'exposes -UserName, -InputCsv, -RemoveHomeDirectory and -NonInteractive' {
        $script:source | Should -Match '\[string\[\]\]\$UserName'
        $script:source | Should -Match '\[string\]\$InputCsv'
        $script:source | Should -Match '\[switch\]\$RemoveHomeDirectory'
    ...
    It 'resolves work in precedence CSV > UserName > dialog, and throws when headless with neither' {
        $script:source | Should -Match '(?s)if\s*\(\$InputCsv\)[\s\S]*?elseif\s*\(\$UserName\)[\s\S]*?elseif\s*\(\s*\$Interactive\s*\)[\s\S]*?Read-MultiLineInputBoxDialog[\s\S]*?else[\s\S]*?throw'
    }
    It 'ingests the reviewed CSV by its SamAccountName column' {
        $script:source | Should -Match '\(Import-Csv\s+\$InputCsv\)\.SamAccountName'
    }
    It 'leaves the home directory in place unless -RemoveHomeDirectory is set' {
        $script:source | Should -Match '(?s)if\s*\(-not\s+\$RemoveHomeDirectory\)[\s\S]*?leaving'
    }
}
...
Describe 'DeleteUsers.ps1 cloud (Graph) branch' {
    ...
    It 'removes directory-role assignments before removing the user' {
        $assignment = ($script:source -split "`n" | Select-String 'Remove-MgRoleManagementDirectoryRoleAssignment' | Select-Object -First 1).LineNumber
        $userRemove = ($script:source -split "`n" | Select-String 'Remove-MgUserByUserPrincipalName' | Select-Object -First 1).LineNumber
        $assignment | Should -Not -BeNullOrEmpty
        $userRemove | Should -Not -BeNullOrEmpty
        $assignment | Should -BeLessThan $userRemove
    }
    It 'gates the cloud-account deletion behind ShouldProcess, before the user removal' {
        $script:source | Should -Match '(?s)\$PSCmdlet\.ShouldProcess\([^\)]*[Cc]loud[\s\S]*?Remove-MgUserByUserPrincipalName'
    }
