...
3 - when running the setup scripts the -Domain param is the NetBIOS name of the domain 
    only, & the -DomainSuffix param is the external domain. EG if you supply -Domain 
...
To Run -
    ...
    8 - Optional - once DC has been patched with latest updates (post April 2023), run \Scripts\Prelim\LAPS.ps1
    9 - Optional - on the router VM run \Scripts\WDS\wds.ps1, then mount ISOs for Server 
        2019 & Win10, & run the corresponding .PS1 script to set up the install 
